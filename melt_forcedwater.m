function [M, T_sh, T_fp, a] = melt_forcedwater(T_far, S_far, P_base, U_rel,DT)
...
a(1) = -5.73e-2; % S contribution to freezing temp 
a(2) = 8.32e-2;  % constant
a(3) = -7.61e-4; % pressure contribution in C/dbar
% so T_fp = a(1)*S_base + a(2) + a(3)*P_base (where S_base,P_base are at base of iceberg)

% changing these Gamma's to Jenkins made fit better, especially at depth
GT = 0.011*0.0097^(1/2); %1.1e-3,6e-4; %second value in Silva, first in Jenkins model  % heat transfer coefficient
GS = 3.1e-4*0.0097^(1/2); %3.1e-5,2.2e-5; %second value in Silva, first in Jenkins model % salt transfer coefficient
L = 3.34e5;  % latent heat of fusion of ice (in J/kg)
...
T_fp = a(1).*S_far + a(2) + a(3).*P_base; % fp temp
T_sh = T_far - T_fp; % temp above fp 
% terms in quadratic
A = (L + DT.*ci)./(U_rel.*GT.*cw);  % term on quadratic
B = -((L + DT.*ci).*GS./(GT.*cw) - a(1).*S_far - T_sh);
C = -U_rel.*GS.*T_sh; 
