...
if exist([dir_output,region_abbrev,'_',DEM1.time,'-',DEM2.time,'_iceberg_melt.mat']) == 2 %file already exists!
    restart_flag = questdlg('Load existing file or start from scratch?',...
        'Reload data','Load Existing','Restart','Load Existing');
    switch restart_flag
        case 'Load Existing'
...
if entry_point <= 1
    ...
    %save the compiled data
    disp('...saving data from earlier DEM...');
    save([dir_output,region_abbrev,'_',DEM1.time,'-',DEM2.time,'_iceberg_melt.mat'],'SL','-v7.3');
    disp('...advance to the later date');
...
if entry_point <= 2
    ...
    %save the compiled data
    disp('...saving data from later DEM...');
    save([dir_output,region_abbrev,'_',DEM1.time,'-',DEM2.time,'_iceberg_melt.mat'],'SL','-v7.3');
    disp('...advance to differencing...');
    close(figure1); close(figure2); clear A;
end
