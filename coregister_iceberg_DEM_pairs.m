function [DEM1,DEM2] = coregister_iceberg_DEM_pairs(DEM1,DEM2,IM1,IM2,region_abbrev,dir_TMD,dir_output)
...
%   - ps2wgs.m
%   - tmd_tide_pred.m
%   - extract_Antarctic_geoid_elevs.m
...
if ~isfile([dir_output,region_abbrev,'_tidal_coords.txt'])
    disp('Creating text files specifying where the tidal info should be extracted');
    disp('     & the area over which geoid elevations should be pulled...');
    ...
    [ROIlon, ROIlat] = ps2wgs([x_min x_max x_max x_min],[y_min y_min y_max y_max],'StandardParallel',-71,'StandardMeridian',0);
...
[geoid] = extract_Antarctic_geoid_elevs(ULlat,ULlon,LRlat,LRlon,region_abbrev,dir_output); 
...
center_coords = dlmread([dir_output,region_abbrev,'_tidal_coords.txt'],'\t');
center_lon = center_coords(1); center_lat = center_coords(2); 

% calculate offset between DEMs
[DEM1,DEM2] = calculate_DEM_offset(DEM1,DEM2,IM1,IM2,dir_output,dir_TMD,center_lon,center_lat);
close all;
