...
disp('Draw a polygon around the map extent containing elevations (non-white areas) in the earlier DEM');
Z_extent = roipoly;
clf(figure1);
disp('Draw a polygon around the map extent containing elevations (non-white areas) in the later DEM');
...
if isnan(nanmean(z_overlap(z_overlap>0))) || isempty(y_overlap) || isempty(x_overlap) || double(size(y_overlap,2)./size(DEM2.z_masked,1))<0.1 || double(size(x_overlap,2)./size(DEM2.z_masked,2))<0.1
    ...
    %Early image
    disp('Draw a polygon around a big relatively ice-free portion of the fjord in each DEM');
    figure1 = figure;
    imagesc(DEM1.x,DEM1.y,DEM1.z_masked); hold on;
    colormap(gca,elev_cmap); colorbar;
...
%     set(gca,'ydir','normal');
    ...
    set(gca,'ydir','normal'); colormap(gca,elev_cmap); colorbar;
...
%     set(gca,'ydir','normal'); 
    ...
    bedcheck = questdlg('Is there bedrock in the overlapping DEM?',...
        'Bedrock check','1) Yes!','2) No','2) No');
    switch bedcheck
        case '1) Yes!'
            %Region 1 mask
