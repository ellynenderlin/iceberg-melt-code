...
while q %use ice-free polygons to remove DEM bias (pick twice)
    ...
    disp('If the adjustment is good: random noise will cause some pixels at sea level to be slightly <0 (white) & some slightly >0 (dark blue)');
    sea_level = questdlg('Are the sea level corrections reasonable?',...
